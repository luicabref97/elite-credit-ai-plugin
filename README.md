...
> Powered by the Elite Credit API v3.2 (106-rule audit + 756-chunk legal RAG over MCP HTTP).
...
- **106 compliance rules** detecting FCRA / FDCPA / Reg F / Reg V / Metro2 violations — including **cross-bureau** (DOFD/balance/status/past-due/high-credit mismatches) and **temporal** (re-aging, reinsertion, repollution)
...
Connect the API to unlock 106 programmatic rules, the 756-chunk legal RAG, cross-bureau and temporal audits, and the 11 Metro2 config files.
...
| `/audit` | Run the 106 compliance rules on previously extracted data. Accepts cross-bureau and temporal payloads. |
...
| `fcra-compliance-auditor` | 106-rule compliance engine with cross-bureau, temporal, and dispute-history awareness. |
| `dispute-strategist` | P0-P4 dispute strategy generation with the 106-rule catalog and 2024-2026 jurisprudence (Spokeo, TransUnion v. Ramirez, Henson, Cushman, Sessa, Hunstein, Heintz, Jerman). |
