---
description: Run 97 FCRA/FDCPA/Reg F/Metro2 compliance rules against credit data (cross-bureau and temporal supported)
---
...
- 82 single-account rules (always)
- 6 collection-specific rules (always when `collections[]` non-empty)
- 8 temporal rules + 1 soft-inquiry temporal rule (when `previous_report_data` is sent)
- 5 cross-bureau inline rules (when `other_bureau_reports` is sent)

Up to **97 registered rules + 5 cross-bureau rules executed inline**.
