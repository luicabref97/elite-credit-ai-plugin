...
Start an interactive credit health Q&A session using previously analyzed credit data and the **527-chunk legal RAG** (17 federal laws + 5 state laws).
...
If the Elite Credit API is connected (MCP server `elite-credit-api`), call `POST /api/rag/search` for detailed legal references from the **527-chunk knowledge base** (rate limit 120/min). Useful category filters:
