...
description: >
  ...
  strategy generation with 527-chunk legal RAG, and dispute-letter generation. Multi-bureau
...
Delegate to the `dispute-strategist` skill, which calls `POST /api/rag/search` if the API is connected to retrieve legal citations from the 527-chunk knowledge base.
