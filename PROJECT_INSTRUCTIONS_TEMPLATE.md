...
3. Use the executing agents only after routing is established:
   - `credit-forensic-analyst` for audits. Produces THREE deliverables every time — `output/forensic_report.md` (technical), `output/consumer_dashboard.md` (plain language), and the interactive single-file dashboard emitted inline as an artifact (also saved to `output/dashboard.html`) with a dedicated print-PDF layout. It runs a Step 8 gate that verifies all three plus the post-audit interview BEFORE asking any closing question.
