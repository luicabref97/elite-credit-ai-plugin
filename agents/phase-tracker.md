...
model: sonnet
color: green
# tools NOT restricted — needs MCP tools (health_check, rag_search) from elite-credit-api. A tools: allowlist excludes MCP when run as a subagent. Omitting tools: inherits all incl. MCP (official Cowork pattern).
---
...
- **NEVER** start a response with API status confirmations ("✅ API online — 756 chunks · 106 rules"). The user does not benefit from this.
